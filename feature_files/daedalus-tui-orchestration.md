...
- **Interpreter discovery**: Convention-based pytest commands resolve the interpreter at discovery time, preferring `python` on `PATH`, then `python3`, then the running interpreter, so hosts that only ship `python3` do not fail verification with a missing-executable error.
